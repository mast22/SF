...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--no-pretty', dest='no_pretty',
            default=False, action='store_true',
            help='By default generated file is human-readable. This option disable it.'
        )

    def write_schema(self, schema, stream, format, pretty=True):
        if format == 'json':
            codec = OpenAPICodecJson(validators=[], pretty=pretty)
    ...
    def handle(self, output_file, overwrite, format, api_url, mock, api_version, user, private, generator_class_name,
        ...
        if output_file == '-':
            self.write_schema(schema, self.stdout, format, pretty)
        else:
            flags = "w" if overwrite else "x"
            with open(output_file, flags) as stream:
                self.write_schema(schema, stream, format, not no_pretty)
