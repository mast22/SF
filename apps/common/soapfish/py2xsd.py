...
def generate_xsd(schema, schema_name='schema'):
    xsd_schema = generate_xsdspec(schema)

    xmlelement = etree.Element(
        '{%s}%s' % (ns.xsd, schema_name),
        nsmap={
...
def schema_validator(schemas):
    ...
    if len(schemas) > 1:
        ...
        # schema_xml = etree.tostring(generate_combined_xsd(schemas))
        # schema_xml = b''.join(etree.tostring(generate_xsd(s, 'import')) for s in schemas)
        # schema_xml = b'<xsd:schema xmlns:xsd="http://www.w3.org/2001/XMLSchema">' + schema_xml + b'</xsd:schema>'
    # else:
    #     schema_xml = etree.tostring(generate_xsd(schemas[0]))
    schema_xml = b''.join(etree.tostring(generate_xsd(s)) for s in schemas)
    # schema_xml = b'<types>' + schema_xml + b'</types>'
    schema_element = etree.fromstring(schema_xml, parser)
    xml_schema = etree.XMLSchema(schema_element)
    return xml_schema.assertValid


def generate_combined_xsd(schemas):
    xmlelement = etree.Element('{%s}schema' % ns.xsd, nsmap={'xsd': xsdspec.XSD_NAMESPACE})
    for schema in schemas:
        xsd_schema = generate_xsdspec(schema)
        xml_subelement = etree.Element(
            '{%s}import' % ns.xsd,
            attrib={
        ...
        xsd_schema.render(xml_subelement,
              xsd_schema,
              namespace=xsdspec.XSD_NAMESPACE,
              elementFormDefault=xsd.ElementFormDefault.QUALIFIED)
        del xml_subelement.attrib['targetNamespace']
        del xml_subelement.attrib['elementFormDefault']
        xmlelement.append(xml_subelement)
    return xmlelement
# --- Program -----------------------------------------------------------------
